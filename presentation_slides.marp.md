...
> into that trained ear, covering **industrial motors down to micro-hardware fans**.
...
```
 Audio      FFT / STFT      Feature Extraction      RandomForest      Health Index
 (mic/wav)   (spectrum)     MFCC · Centroid ·        Classifier        0-100% + badge
                             Rolloff · ZCR · RMS
```

- **Signal processing:** Librosa / SciPy, time-domain and frequency spectrogram.
- **ML model:** Scikit-learn RandomForest, labels `healthy` / `friction` / `imbalance`.
- **Dashboard:** Streamlit, live mic or `.wav`, plots, and actionable advice.
- **Data:** synthetic generator trains and demos with **zero physical hardware**.
...
- **Extends equipment life:** catch faults early, avoid catastrophic breakdowns and e-waste.
- **Democratises maintenance:** no cloud, no subscriptions, runs on a laptop.
- **Privacy-first and offline:** audio never leaves the device.
- **Broadly applicable:** factory motors, HVAC, medical devices, farm pumps, micro-hardware.

## GridSense: predictive maintenance for **everyone**, from family workshops to Fortune 500 factories.
