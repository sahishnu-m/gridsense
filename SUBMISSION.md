...
- **Fully offline and private:** audio never leaves the device.
...
and reduces electronic waste. It runs on commodity hardware with no recurring
cost, so it's deployable in resource-constrained settings worldwide: factory
motors, HVAC, medical devices, farm pumps, and micro-hardware alike.
...
- [ ] **Demo:** a 1-3 minute video (see `DEMO_SCRIPT.md`) OR the detailed text above.
